...
export default () => {
  ...
  const deleteUserById = async id => {
    ...
    try {
      await deleteUser(id);
      open({ message: 'משתמש נמחק בהצלחה!', severity: severities.success });
    } catch ({
      response: {
        data: { message },
      },
    }) {
      open({ message, severity: severities.error });
    }
  };
  ...
  const addUser = async newUserDetails => {
    try {
      ...
      open({ message: 'משתמש נוסף בהצלחה', severity: severities.success });
    } catch ({
      response: {
        data: { message },
      },
